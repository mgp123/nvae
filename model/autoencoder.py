...
class Autoencoder(nn.Module):
    def __init__(self,
        ...
        if use_mix:
            # mix of 10 distributions each with 3 parameters and a parametr for the mixture probabilities
            d_parameters = 10*10 
    ...
    def sample(self, n):
        ...
        for i in range(self.decoder_tower.n_inputs):
            # note that the mixing for i == 0 behaves different
            residual_dec = self.mixer.decoder_only_mix(residual_dec, i)
            residual_dec = self.decoder_tower(residual_dec, i)
        ...
        x = torch.clamp(x_distribution.sample(), -1, 1.) 
        x = x / 2. + 0.5
        return x
