...
from model.utils import Normal, device

# we are always going to have in_channels1==in_channels2
# but to be consistent with the original immplementation we
...
class Mixer(nn.Module):
    ...
    def decoder_only_mix(self, dec_part, i):
        if i!= 0:
            latent = self.decoder_sampler[i](dec_part)
            mu, log_sig = torch.chunk(latent,2,1)
            distribution = Normal(mu, log_sig)
